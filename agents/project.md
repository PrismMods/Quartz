...
- `sdk/` (repo root) — the addon SDK: `QuartzAddon.dll` (reference assembly, regenerated by the build via `ProduceReferenceAssembly` + the `PublishAddonSdk` target), `QuartzAddon.props`, README. Modders build `.qaddon`s against it.
