...
// Ported from tjwogud's "FlipAndRotateTiles" (UMM). The game already binds the
// editor shortcuts (Ctrl+L / Ctrl+Shift+L flip, Ctrl+, / Ctrl+. rotate 90°,
// Ctrl+/ rotate 180°); these postfixes ride the resulting scnEditor methods to
//   1) mirror/rotate the positionOffset stored in each tile's Position Track
//      events so attached positions follow the tiles, and
//   2) optionally rotate by a custom angle instead of the fixed 90°.
public static partial class EditorFeature {
    ...
    // True only while an original RotateFloor body is executing, so the
    // GetRotDirection prefixes rewrite the rotation angle exclusively for that
    // path and leave every other GetRotDirection caller (e.g. Nostalgia's
    // Space360 tile creation) untouched.
    ...
    private static bool TrySelectionRange(out int first, out int last) {
        ...
        // Floor 0 is the immovable start tile; the source mod bails on it.
        return first != 0;
    }

    // Applies a Vector2 transform to the positionOffset of every Position Track
    // event whose floor lies in [first, last]. Single-floor callers pass the
    // same seqID for both bounds.
    ...
    private static class FlipFloorPatch {
        // FlipSelection calls this with remakePath:false per floor, so the
        // remakePath gate keeps the per-floor and whole-selection paths from
        // both firing on the same op.
    ...
    private static class RotateFloorPatch {
        ...
        // Finalizer always runs, even if the original throws, so the flag can
        // never latch on and bleed a custom angle into unrelated rotations.
    ...
    // 24-step (15° each) ring of legacy string directions; custom angle steps
    // around it for old-format (pathData) levels.
    ...
    // New-format (angleData) levels store the angle as a float; 999 is the
    // "midspin" sentinel and must pass through unchanged.
