...
public static partial class PlanetColors {
    ...
    [HarmonyPatch]
    private static class EditorPlayModePatch {
        private static IEnumerable<MethodBase> TargetMethods()
            => ExistingMethods(typeof(scnEditor), "Play", "SwitchToEditMode");
        private static void Postfix() {
            if(ShouldChange) Refresh();
            else DisableAllOverlays();
        }
    }
    [HarmonyPatch]
    private static class SceneResetPatch {
        private static IEnumerable<MethodBase> TargetMethods()
            => ExistingMethods(typeof(scnGame), "ResetScene");
        private static void Postfix() {
            if(ShouldChange) Refresh();
            else DisableAllOverlays();
        }
