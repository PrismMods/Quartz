    ...
    private sealed class Updater : MonoBehaviour {
        ...
        private void UpdatePanel(LivePanel p, bool show, bool isReorganizing, bool refreshText) {
            ...
            if(fontChanged || p.Dirty || body != p.LastBody) {
                p.Text.text = body;
                Vector2 pref = p.Text.GetPreferredValues();
                Vector2 size = new(pref.x + PadX * 2f, pref.y + PadY * 2f);
                if(p.Rect.sizeDelta != size) p.Rect.sizeDelta = size;
                TMPTextShadow.Apply(
                    p.Text,
        ...
        private static StatDef FindStat(string id) => PanelsOverlay.FindStat(id);
