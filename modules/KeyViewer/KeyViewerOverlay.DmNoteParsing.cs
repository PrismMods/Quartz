...
public static partial class KeyViewerOverlay {
    ...
    private static void ApplyDmRuntimeSettings() {
        ...
        tabubEnabled = Conf.TabubEnabled;
        tabubPercent = Mathf.Clamp(Conf.TabubPercent, 0f, 100f);
    }
    ...
    private static DmNoteSpec ParseDmNoteSpec(string keyName, JObject p, bool stat) {
        ...
        spec.KeyInlineStyles = JBool(p, "useInlineStyles", false);
    ...
    private static KeyCode ResolveDmNoteKeyCode(string name) {
        ...
        if(name.Length > 1 && int.TryParse(name, out int numeric)) return KeyCodes.NormalizeNumeric(numeric);
        string normalized = name.Replace(" ", "").Replace("_", "").Replace("-", "");
        if(normalized.Length == 0) return KeyCode.None;
        ...
        if(!char.IsDigit(normalized[0]) && Enum.TryParse(normalized, true, out KeyCode parsed)) return KeyCodes.Normalize(parsed);
