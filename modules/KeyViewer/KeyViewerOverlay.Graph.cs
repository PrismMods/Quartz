...
public static partial class KeyViewerOverlay {
    ...
    private sealed class KpsGraphGraphic : MaskableGraphic {
        ...
        private void Update() {
            ...
            if(now >= _nextSample) {
                float step = GraphUpdateMs / 1000f;
                int shift = 1;
                if(_lastSample >= 0f) {
                    shift = Mathf.Clamp(Mathf.FloorToInt((now - _lastSample) / step), 1, _animTo.Length);
                }
                _lastSample = now;
                Sample(shift);
                _nextSample = now + step;
            }
            if(_anim && _animStart >= 0f) {
                ...
                for(int i = 0; i < _history.Length; i++) {
                    float v = Mathf.Lerp(_animFrom[i], _animTo[i], e);
                    if(!moved && Mathf.Abs(v - _history[i]) > 0.0005f) moved = true;
                    _history[i] = v;
                }
        ...
        private void Sample(int shift) {
            ...
            int len = _animTo.Length;
            int keep = len - shift;
            if(keep > 0) Array.Copy(_animTo, shift, _animTo, 0, keep);
            for(int i = Mathf.Max(0, keep); i < len; i++) _animTo[i] = value;
