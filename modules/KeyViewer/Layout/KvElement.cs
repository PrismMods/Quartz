...
internal sealed partial class KvElement {
    ...
    private string StrEither(string key, string fallback) {
        JObject outer = Container;
        if(!ReferenceEquals(outer, Raw) && outer[key] is { Type: not JTokenType.Null } t && t.ToString().Length > 0)
            return t.ToString();
        return Str(key, fallback);
    }
    private void SetEither(string key, JToken value) {
        JObject outer = Container;
        bool nested = !ReferenceEquals(outer, Raw);
        if(value == null) {
            Raw.Remove(key);
            if(nested) outer.Remove(key);
            return;
        }
        Raw[key] = value;
        if(nested) outer[key] = value.DeepClone();
    }
    internal float X {
        get => Num("dx", 0f);
    ...
    internal string ClassName {
        get => StrEither("className", "");
        set => SetEither("className", string.IsNullOrWhiteSpace(value) ? null : (JToken)value.Trim());
    }
    internal bool UseInlineStyles {
        get => FlagEither("useInlineStyles", false);
        set => SetEither("useInlineStyles", value ? (JToken)true : null);
    }
