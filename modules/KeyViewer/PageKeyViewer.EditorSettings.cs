...
internal static partial class PageKeyViewer {
    private static Action AppendEditorSettings(
        RectTransform body, KeyViewerSettings conf, KvCanvas canvas, Action refreshTabs
    ) {
        KeyViewerSettings def = new();
        KvWidgets.Header(body, "KEYVIEWER_EDITOR_TAB_NAME", "Tab Name");
        UIInput tabName = KvWidgets.Input(
            GenerateUI.Row(body), "", KvStore.Current.TabName(canvas.Tab),
        ...
        tabName.InputField.onEndEdit.AddListener(raw => {
            ...
            string result = doc.RenameTab(canvas.Tab, raw);
            tabName.InputField.SetTextWithoutNotify(result ?? doc.TabName(canvas.Tab));
        ...
        return () => {
            tabName.InputField.SetTextWithoutNotify(KvStore.Current.TabName(canvas.Tab));
