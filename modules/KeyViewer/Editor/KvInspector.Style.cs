...
internal sealed partial class KvInspector {
    ...
    private void CssRows(RectTransform root, List<UIObject> tracked, KvElement[] batch) {
        ...
        Flag(
            root, tracked, "Inline Styles Priority", "kvi_inline_priority", false,
            batch, el => el.UseInlineStyles,
            (el, v) => el.UseInlineStyles = v
        ).Rect.AddToolTip(
            "DESC_KVI_INLINE_PRIORITY",
            "Keep this element's own colours, sizes and press scale even when the custom stylesheet sets them. CSS-only effects such as gradients, glows and pseudo layers still apply."
        );
        TextField(
            root, tracked, batch, "kvi_class", "Class (custom CSS)",
            el => el.ClassName, (el, v) => el.ClassName = v, ClassNameLimit
        ).Rect.AddToolTip(
            "DESC_KVI_CLASS",
            "Space-separated class names for the custom stylesheet. A rule such as .blue[data-state=\"active\"] then targets this element."
        );
    }
